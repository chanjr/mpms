...
class RawScanViewer:
    def __init__(self, mpms, fig = None):
        self.mpms = mpms
    ...
    def create_viewer(self, fig = None):
        ...
        self.scan_slider = Slider(ax = slider_ax, label = 'Scan #', valmin = 0,
                valmax = len(self.mpms.raw) - 1, valstep = 1)
        ...
        # original_ax
        self.raw_line, = original_ax.plot(self.mpms.position[0], self.mpms.voltage[0],
                label = 'Raw data', marker = '.', linestyle = '')
        self.fit_line, = original_ax.plot(self.mpms.position[0], self.mpms.raw_fit_voltage[0],
                label = 'MPMS fit', marker = '', linestyle = '-', alpha = 0.8)
        self.background_line, = original_ax.plot(self.mpms.position[0], self.mpms.background[0],
                label = 'Background', marker = '', linestyle = '-', alpha = 0.8)
        self.original_dipole_line, = original_ax.plot([], [],
                label = 'Dipole', marker = '', linestyle = '-', alpha = 0.8)
        self.center_marker_o = original_ax.axvline(self.mpms.scan_amplitude[0]/2, color = 'C5')

        # refit_ax
        self.raw_line_corrected, = refit_ax.plot(self.mpms.position[0], self.mpms.voltage[0],
                label = 'Raw - background', marker = '.', linestyle = '')
        self.refit_line, = refit_ax.plot([], [],
                label = 'Refit', marker = '', linestyle = '-', alpha = 0.8)
        self.refit_dipole_line, = refit_ax.plot([], [],
                label = 'Dipole', marker = '', linestyle = '-', alpha = 0.8)
        self.center_marker_r = refit_ax.axvline(self.mpms.scan_amplitude[0]/2, color = 'C5')
        ...
        self.data_line = self.mpms.plot_dat(ax = moment_ax, marker = '.', picker = True,
                pickradius = 5, label = 'Original')
        self.picked_data_line, = moment_ax.plot([], [], marker = 'o', linestyle = '')
        self.refit_moment_line, = moment_ax.plot([], [], marker = '.', label = 'Refit')

        # fit_value_ax
        self.fit_value_line = self.mpms.plot_fit_value(ax = fit_value_ax, marker = '.',
                picker = True, pickradius = 5, label = 'Original fit value')
        self.picked_fit_value_line, = fit_value_ax.plot([], [], marker = 'o',
                linestyle = '')
    ...
    def select_scan_idx(self, idx):
        ...
        self.fig.suptitle(f'{self.mpms.root}.raw\nScan {idx}, T = {T:.1f} K, H = {H:.1f} Oe')
        for ax in self.axes.flatten():
            rescale_visible(ax)
    ...
    def add_dipole_background(self, event):
        ...
        self.dipole_moment = self.mpms.voltage[self.viewer_idx].ptp()/2.4
        self.dipole_center = self.center
        x = self.mpms.position
        y = dipole_response(self.dipole_moment, self.dipole_center)
        self.original_dipole_line.set_data(x, y)
        self.refit_dipole_line.set_data(x, y)

    def fit_dipole_background(self, event):
        logger.info(f'Adding dipole background from Scan {self.viewer_idx} at {self.center:.2f}')
