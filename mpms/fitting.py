...
def curie_weiss(T, H = 200, T_C = 50, N = 1e17, M_0 = 0, g = 2, J = 7/2):
    ...
    return np.piecewise(T, [T < T_C, T >= T_C],
...
def signed_interpolation(x, y):
    for i, sgn in monotonic_slices(x):
        ...
        yield scipy.interpolate.interp1d(x_i, y_i, kind = 'slinear', fill_value = 'extrapolate'), sgn, x_i, y_i

def estimate_coercivity(H, M):
    for f, s, H_i, M_i in signed_interpolation(H, M):
        if not (M_i.min() < 0 < M_i.max()):
            logger.warn(f'Magnetic moment does not cross zero ({M_i.min():.3e} -> {M_i.max():.3e})')
            continue
...
def estimate_remanence(H, M):
    for f, s, H_i, M_i in signed_interpolation(H, M):
        if not (H_i.min() < 0 < H_i.max()):
            logger.warn(f'Applied field does not cross zero ({H_i.min():.3e} -> {H_i.max():.3e})')
            continue
        yield f(0), s
