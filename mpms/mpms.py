...
from .fitting import fit_inverse_susceptibility
from .fitting import estimate_coercivity
from .fitting import estimate_remanence
from .fitting import dipole_response
from .fitting import n_dipole_model
from .fitting import r_square
...
class FilmSample(Sample):
    def __init__(self, sample_mass, substrate, substrate_thickness, film_thickness, n_density = None):
        # cgs units
    ...
    def load_json(cls, fname):
        with open(fname, 'r') as f:
            data = json.load(f)
        return cls(data['sample_mass'], reference.substrates[data['substrate']],
                data['substrate_thickness'], data['film_thickness'],
...
class MPMS:
    ...
    def __init__(self, fname, sample = None):
        ...
        ext_prog = re.compile(f'({"|".join(re.escape("." + ext) for ext in file_extensions)})$')
    ...
    def parse_mpms(fname):
        ...
        try:
            with open(fname, 'r') as f:
                ...
                reader = csv.reader(f)
                data = np.genfromtxt(('\t'.join(row) for row in reader),
                        delimiter = '\t', names = True)
        except FileNotFoundError:
            logger.warn(f"Error parsing {fname}: file not found")
            return None, None
        used_columns = [c for c in data.dtype.names
                if ~np.all(np.isnan(data[c]))]
    ...
    def raw(self):
        ...
        if _raw is None:
            return None
        scaled_ptp = _raw['Long_Scaled_Response'].ptp(axis = 1)
        unscaled_ptp = _raw['Long_Voltage'].ptp(axis = 1)
    ...
    def set_scan_background(self, idx, center, n_dipoles = 1, **kwargs):
        ...
        m0 = self.voltage[idx].ptp()/2.4 # initial estimate for moment
        res = model.fit(self.voltage[idx], x = self.position[idx],
                d0_center = center, d0_moment = m0, **kwargs)
        self.add_dipole_background(res.best_values['d0_moment'],
                res.best_values['d0_center'])
    ...
    def scan_fit(self, idx, d0_center, n_dipoles = 1, model = None, **kwargs):
        ...
        m0 = self.voltage[idx].ptp()/2.4 # initial estimate for moment
        res = model.fit(self.voltage[idx], x = self.position[idx],
                d0_center = d0_center, d0_moment = m0, **kwargs)
        self.fits[idx] = res
        return res
    ...
    def plot_dat(self, ax = None, label = None, min_fit = None, **kwargs):
        ...
        if min_fit is not None:
            ...
            if n_missed:
                logger.warning(f'{n_missed} points with fit value less than {min_fit} in {self.root}.dat')
        else:
            idx = slice(None)
        if self.field_dependent:
            return ax.plot(self.field[idx], self.magnetization[idx], label = label, **kwargs)[0]
        else:
            ...
            return ax.plot(self.temperature[idx], self.magnetization[idx], label = label, **kwargs)[0]
    ...
    def plot_inverse_susceptibility(self, ax, **kwargs):
        if self.field_dependent:
            logger.warning(f'{self.root}.dat is not a temperature-dependent measurement')
            return
        if self.field[0] == 0:
        ...
        s = np.sign(self.temperature[-1] - self.temperature[0]) < 0
        label = f"{['ZFC', 'FC'][s]} {self.field[0]:.1f} Oe"
        return ax.plot(self.temperature, self.field[0]/self.magnetization, label = label, **kwargs)[0]

    def estimate_curie_temperature(self, window_length = 11, polyorder = 3):
        idx = savgol_filter(self.moment, window_length, polyorder, deriv = 2).argmax()
    ...
    def fit_curie_weiss(self, ax = None, T_C = None, ion = 'Gd', T_max = 300,
            update_offset = False, **params):
        if not self.temperature_dependent:
            logger.warn(f'Curie-Weiss can only fit temperature dependence ({self.root}.dat)')
            return
        if T_C is None:
            T_C = self.estimate_curie_temperature()
        r_e = reference.ground_states[ion]
        idx = self.temperature < T_max
        res, res2, M_0 = fit_inverse_susceptibility(self.temperature[idx], self.moment[idx],
                H = self.field[0], T_C = T_C, g = r_e.g, J = r_e.J, **params)
        if update_offset:
        ...
        ax.plot(res2.userkws['T'], res2.data,
                label = label, marker = '.')
        ax.plot(res2.userkws['T'], res2.best_fit,
...
def plot_combined(mpms_list, fig = None, T_max = 300, min_fit = 0, fit_curie_weiss = True):
    ...
    field_dependent = sorted([mpms for mpms in mpms_list if mpms.field_dependent], key = lambda x: x.temperature[0])
    for mpms in field_dependent[:]:
        if mpms.temperature_spread > 1:
            logger.warn(f'Temperature spread in {mpms.root} too large ({mpms.temperature_spread} K)')
    ...
    norm = mpl.colors.Normalize(vmin = field_dependent[0].set_temperature,
            vmax = field_dependent[-1].set_temperature*1.3)
    for mpms in field_dependent:
        t = mpms.set_temperature
        mpms.plot_dat(ax = ax2, color = cmap(norm(t)), label = f'{t} K', marker = '.', min_fit = min_fit)
        mpms.plot_dat(ax = ax5, color = cmap(norm(t)), label = f'{t} K', marker = '.', min_fit = min_fit)
        mpms.estimate_coercivity()
        mpms.estimate_remanence()
    H_ci = np.array([(mpms.set_temperature, h_c[0]) for mpms in field_dependent
        for h_c in mpms.coercivities if h_c[1] == 1])
    H_cd = np.array([(mpms.set_temperature, h_c[0]) for mpms in field_dependent
        for h_c in mpms.coercivities if h_c[1] == -1])
    M_ri = np.array([(mpms.set_temperature, m_r[0]) for mpms in field_dependent
        for m_r in mpms.remanences if m_r[1] == 1])
    M_rd = np.array([(mpms.set_temperature, m_r[0]) for mpms in field_dependent
        for m_r in mpms.remanences if m_r[1] == -1])
    ...
    xlim = 1.5*np.abs([h_c[0] for mpms in field_dependent
        for h_c in mpms.coercivities]).max()
    ylim = 1.3*np.abs([m_r[0] for mpms in field_dependent
        for m_r in mpms.remanences]).max()
